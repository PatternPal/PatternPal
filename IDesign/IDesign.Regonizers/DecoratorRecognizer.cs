    ...
    public class DecoratorRecognizer : IRecognizer
    {
        ...
        public IResult Recognize(IEntityNode entityNode)
        {
            ...
            var decoratorCheck = new GroupCheck<IEntityNode, IEntityNode>(new List<ICheck<IEntityNode>>()
            {
                ...
                new ElementCheck<IEntityNode>(x => x.CheckMinimalAmountOfRelationTypes(RelationType.Extends, 1) ||
                x.CheckMinimalAmountOfRelationTypes(RelationType.Implements, 1), "Has an interface or an parent class"),
                ...
                {
                    new ElementCheck<IEntityNode>(x => {currentComponent = x; return x.GetMethodsAndProperties().Any(); }, "Has functions"),
                    new ElementCheck<IEntityNode>(x => x.CheckMinimalAmountOfRelationTypes(RelationType.ExtendedBy, 2) ||
                    x.CheckMinimalAmountOfRelationTypes(RelationType.ImplementedBy, 2), "Extended or implemented by a different class"),
                    ...
                    new GroupCheck<IEntityNode, IEntityNode>(new List<ICheck<IEntityNode>>
                    {
                        new GroupCheck<IEntityNode, IMethod>(new List<ICheck<IMethod>>
                        {
                            ...
                            new ElementCheck<IMethod>(x => x.CheckIfArgumentsExists(currentComponent.GetName()), "Sends the interface or parent class parameter in base")
                ...
                }, x => x.GetRelations().Where(y => y.GetRelationType().Equals(RelationType.Implements) || 
                y.GetRelationType().Equals(RelationType.Extends)).Select(y => y.GetDestination()), "Interface or parent class")
