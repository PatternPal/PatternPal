...
namespace IDesign.Recognizers
{
    public class DecoratorRecognizer : IRecognizer
    {
        ...
        public IResult Recognize(IEntityNode entityNode)
        {
            ...
            var decoratorCheck = new GroupCheck<IEntityNode, IEntityNode>(
                new List<ICheck<IEntityNode>>
                {
                    ...
                    new ElementCheck<IEntityNode>(
                        x => x.CheckMinimalAmountOfRelationTypes(RelationType.ExtendedBy, 1), "Child", 1
                    ),
                    ...
                    new GroupCheck<IEntityNode, IEntityNode>(
                            ...
                            new GroupCheck<IEntityNode, IEntityNode>(
                                new List<ICheck<IEntityNode>>
                                {
                                    new GroupCheck<IEntityNode, IMethod>(
                                        ...
                                        {
                                            new ElementCheck<IMethod>(
                                                x => x.CheckModifier("public"), "MethodModifierPublic", 1
                                            ),
                                            new ElementCheck<IMethod>(
                                                x => x.CheckParameters(
                                                    new List<string> { currentComponent.GetName() }
                                    ...
                                    new GroupCheck<IEntityNode, IMethod>(
                                        ...
                                        {
                                            new ElementCheck<IMethod>(
                                                x => x.CheckIfNameExists(
                                                    currentComponent.GetMethodsAndProperties()
                                                ), "MethodNameOverride"
                                            ),
                                            new ElementCheck<IMethod>(
                                                x => x.CheckIfMethodCallsBase(), "MethodBaseCall"
                                            )
                        ...
                        x => x.GetRelations()
                            .Where(
                                y => y.GetRelationType().Equals(RelationType.Implements) ||
                                     y.GetRelationType().Equals(RelationType.Extends)
                            ).Select(y => y.GetDestination()), "DecoratorComponent"
                    )
