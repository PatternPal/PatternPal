...
namespace IDesign.Recognizers.Recognizers
{
    public class StrategyRecognizer : IRecognizer
    {
        ...
        public IResult Recognize(IEntity node)
        {
            ...
            var strategyPatternCheck = new GroupCheck<IEntity, IEntity>(new List<ICheck<IEntity>>
            {
                //check if node is abstract class or interface
                new ElementCheck<IEntity>(x => (x.CheckTypeDeclaration(EntityType.Interface) ) |
                (x.CheckTypeDeclaration(EntityType.Class) && x.CheckModifier("abstract")),"NodeAbstractOrInterface", 2),

                //check strategy node methods
                new GroupCheck<IEntity, IMethod>(new List<ICheck<IMethod>>
                {
                ...
                {
                    new ElementCheck<IEntity>(x => x.CheckMinimalAmountOfRelationTypes(RelationType.UsedBy, 1),"NodeUses1", 1),

                    //check if field has stratgey as type
                    new GroupCheck<IEntity, IEntity>(new List<ICheck<IEntity>>
                    {
                        new ElementCheck<IEntity>(x => (x.CheckTypeDeclaration(EntityType.Interface)) |
                        (x.CheckTypeDeclaration(EntityType.Class) && x.CheckModifier("abstract")), "NodeAbstractOrInterface", 1)
                ...
                //check inheritance
                 new GroupCheck<IEntity, IEntity>(new List<ICheck<IEntity>>
                 {
                     new ElementCheck<IEntity>(x => !x.GetRelations().Any(y => y.GetRelationType() ==RelationType.Creates),"NodeDoesNotCreate", 2),
                     new ElementCheck<IEntity>(x => !x.GetRelations().Any(y => y.GetRelationType() ==RelationType.Uses), "NodeDoesNotUse", 1),
