    ...
    public class FactoryMethodRecognizer : IRecognizer
    {
        ...
        public IResult Recognize(IEntity node)
        {
            ...
            var factoryMethodChecks = new GroupCheck<IEntity, IEntity>(new List<ICheck<IEntity>>
            {
                ...
                new ElementCheck<IEntity>(x => x.CheckTypeDeclaration(EntityType.Class)&& x.CheckModifier("abstract"),"NodeModifierAbstract", 2),
                new ElementCheck<IEntity>(x=> x.CheckMinimalAmountOfRelationTypes(RelationType.Uses,1),"NodeUses1", 1),

                //check concretecreators
                new GroupCheck<IEntity, IEntity>(new List<ICheck<IEntity>>
                {
                    //concrete creator
                    new ElementCheck<IEntity>(x => {entityNode = x; return x.GetAllMethods().Any(); }, "FactoryConcreteCreatorMethodAny"),

                    //check if node (concrete creator) has creates relations
                    new GroupCheck<IEntity, IEntity>(new List<ICheck<IEntity>>
                    {
                        //concrete product
                        new ElementCheck<IEntity>(x => {productnode = x; return x.GetAllMethods().Any(); }, "FactoryConcreteProductMethodAny"),
                        ...
                        {
                            //product
                            new ElementCheck<IEntity>(x => {productnode = x; return x.GetAllMethods().Any() || x.GetFields().Any(); }, "ProductClass"),
                ...
                {
                    //product
                    new ElementCheck<IEntity>(x => {entityNode = x; return x.GetAllMethods().Any() || x.GetFields().Any(); }, "ProductClassNotEmpty"),
