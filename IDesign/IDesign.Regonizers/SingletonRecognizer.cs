        ...
        public IResult Recognize(IEntityNode entityNode)
        {
            ...
            var propertyChecks = new List<ICheck<IField>>
            {
                new ElementCheck<IField>(
                    x => x.CheckFieldType(new List<string> { entityNode.GetName() }),
            ...
            var singletonCheck = new GroupCheck<IEntityNode, IEntityNode>(
                new List<ICheck<IEntityNode>>
                {
                    new GroupCheck<IEntityNode, IMethod>(
                        methodChecks, x => x.GetMethodsAndProperties(), "SingletonMethod"
                    ),
                    new GroupCheck<IEntityNode, IField>(propertyChecks, x => x.GetFields(), "SingletonField"),
                    new GroupCheck<IEntityNode, IMethod>(
                        constructorChecks, x => x.GetConstructors(), "SingletonConstructor"
                    )
