...
namespace IDesign.Recognizers
{
    public class ObserverRecognizer : IRecognizer
    {
        ...
        private IResult ObserverWithSubjectInterfaceCheck(IEntityNode node)
        {
            ...
            var checks = new GroupCheck<IEntityNode, IEntityNode>(
                new List<ICheck<IEntityNode>>
                {
                    ...
                    new ElementCheck<IEntityNode>(
                        x => x.CheckRelationType(RelationType.ImplementedBy), "NodeImplementedByAny", 0.33f
                    ),
                    new ElementCheck<IEntityNode>(
                        x => x.CheckRelationType(RelationType.UsedBy), "NodeUsedByAny", 0.33f
                    ),
                    new GroupCheck<IEntityNode, IEntityNode>(
                        new List<ICheck<IEntityNode>>
                        {
                            new ElementCheck<IEntityNode>(
                                x => x.CheckMinimalAmountOfMethods(3), "MethodAmountThree", 0.66f
                            ),
                            new ElementCheck<IEntityNode>(
                            ...
                            new ElementCheck<IEntityNode>(
                                x => x.CheckRelationType(RelationType.ImplementedBy), "NodeImplementedByAny",
                                0.33f
                            ),
                            new GroupCheck<IEntityNode, IEntityNode>(
                                new List<ICheck<IEntityNode>>
                                {
                                    new ElementCheck<IEntityNode>(
                                        x => x.CheckTypeDeclaration(EntityNodeType.Class), "NodeTypeClass"
                                    ),
                                    new GroupCheck<IEntityNode, IField>(
                                        new List<ICheck<IField>>
                                        {
                                            new ElementCheck<IField>(
                                                x => x.CheckFieldType(
                                                    new List<string> { $"List<{node.GetName()}>" }
                                                ),
                                                new ResourceMessage(
                                                    "FieldType", new[] { node.GetName() }
                                                ), 1f
                                            )
            ...
            var checks = new GroupCheck<IEntityNode, IEntityNode>(
                new List<ICheck<IEntityNode>>
                {
                    ...
                    new GroupCheck<IEntityNode, IEntityNode>(
                        ...
                        {
                            new ElementCheck<IEntityNode>(
                                x => x.CheckMinimalAmountOfMethods(3), "MethodAmountThree", 0.33f
                            ),
                            new ElementCheck<IEntityNode>(
                                x => x.CheckMinimalAmountOfMethodsWithParameter(
                                    new List<string> { node.GetName() }, 2
                            ...
                            new ElementCheck<IEntityNode>(
                                x => x.CheckEntityNodeType(EntityNodeType.Class), "NodeTypeClass", 0.33f
                            ),
                            new ElementCheck<IEntityNode>(
