    ...
    public partial class ExtensionWindowControl : UserControl, IVsSolutionEvents, IVsRunningDocTableEvents {
        ...
        private void CreateResultViewModels(IEnumerable<RecognitionResult> results) {
            ...
            foreach (var patterns in from item in RecognizerRunner.DesignPatterns
                     ...
                     select patterns) {
                viewModels.AddRange(
                    patterns.OrderBy(x => x.Result.GetScore()).Select(x => new PatternResultViewModel(x))
                );
            }
        ...
        [SuppressMessage(
            "StyleCop.CSharp.NamingRules", "SA1300:ElementMustBeginWithUpperCaseLetter", Justification =
        ...
        private async void Analyse() {
            ...
            if (!Loading && Paths.Count != 0 && SelectedPatterns.Count != 0) {
                if ((bool)SelectPaths.radio1.IsChecked) CheckSwitch.IsChecked = true;
                else CheckSwitch.IsChecked = false;

                var runner = new RecognizerRunner();
                Loading = true;
                statusBar.Value = 0;
                var progress = new Progress<RecognizerProgress>(
                    value => {
                            ...
                            Dispatcher?.BeginInvoke(
                                new Action(
                                    () => {
                                        ...
                                        else SummaryRow.Height = new GridLength(0);
