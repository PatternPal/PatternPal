...
namespace IDesign.Extension
{
    internal class SummaryFactory
    {
        private string CreateFilesAnalysedSummary(
            ...
            IEnumerable<IEntityNode> uniqueEntityNodes
        )
        {
            var result = "";
            ...
            switch (uniqueEntityNodes.Count())
            {
                ...
                default:
                    result +=
                        $"{string.Format(SummaryRescources.MultipleEntiyNodesAnalysed, uniqueEntityNodes.Count())} ";
                    break;
            }
        ...
        public string CreateSummary(IEnumerable<RecognitionResult> results, IEnumerable<RecognitionResult> allResults)
        {
            ...
            var recognizedPatters = results.Where(x => x.Result.GetScore() >= 80)
                .OrderByDescending(x => x.Result.GetScore());
            var recognizedPatternsCount = recognizedPatters.Count();

            var resultList = CreatePartOfPatternSummary(
